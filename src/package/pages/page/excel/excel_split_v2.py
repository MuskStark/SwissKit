...
from .excel_split_keep_format import ExcelSplitKM, ExcelSplitConfig
...
class ExcelSplitPageV2(ToolBoxPage):
    def __init__(self, main_page: ft.Page):
        self.theme = {"text_color": ft.Colors.GREY_600}
        self.file_analyze_dic = {}
        self.page = main_page
        self.disable = True
        self.excel = None
        self.checkBox = ft.Checkbox(label='拆分后打开输出文件夹', value=True)
        self.kf_checkbox = ft.Checkbox(label='保持原文件格式', value=False)
        self.progress = ProgressRingComponent()
        self.page.add(self.progress)
        self.page.update()
        self.kf_editor = ExcelSplitKM(progress_callback=self.progress.update_status)
        ...
        try:
            ...
            if file_path:
                if file_path.suffix == '.xlsx':
                    ...
                    self.kf_checkbox.disabled = True
                    self.page.update()
                else:
    ...
    def _picker(self, e: ft.FilePickerResultEvent, file_path_text: ft.TextField, is_file: bool = True):
        if is_file:
            if e.files:
            ...
            try:
                ...
                for _sheet_name in _split_config_dic.keys():
                    ...
                    split_configs.append({
                        ...
                        'header_rows': _split_config_dic[_sheet_name]['header_index'],
                        'split_column_index': _split_config_dic[_sheet_name]['column_index']
                    })
                result_dic = {}
                ...
                if self.kf_checkbox.value:
                    ...
                    config.split_model = 1
                    config.split_sub_model = 'multiple'
                    config.split_config = split_configs
                    success = self.kf_editor.split_keep_format(self.excel, config, _output_folder_path_text.value)
                    if not success:
                        raise RuntimeError('保持格式拆分失败')
                else:
                    for k, v in result_dic.items():
                        if '/' in k:
                            k = k.replace('/', '-')
    ...
    def _simple_split_excel(self, output_folder_path_text: ft.TextField):
        def redio_on_change(mode, button, sheet_dropdown, columns_dropdown):
            if mode.value == '0':
        ...
        def sheet_dropdown_changed(selector, columns_selector):
        ...
        sheet_selector = ft.Dropdown(label='请选择Sheet页',
                                     on_change=lambda _: sheet_dropdown_changed(sheet_selector, columns_selector),
                                     options=[], width=200, disabled=self.disable)
        columns_selector = ft.Dropdown(label='请选择拆分列', options=[], width=300, disabled=self.disable)
        mode = ft.RadioGroup(
            ft.Row([ft.Radio(value="0", label="根据Sheet拆分"), ft.Radio(value="1", label="根据列拆分")]),
            value="0",
        ...
        return ft.Column(
            controls=[
                ...
                ft.Row([ft.ElevatedButton(text='开始拆分',
                                          on_click=lambda _: business_logic(mode, sheet_selector, columns_selector,
                                                                            output_folder_path_text, handel_progress))],
    ...
    def gui(self):
        ...
        excel_components = ft.Column(controls=[
            ft.Row([ft.IconButton(icon=ft.Icons.UPLOAD_FILE, on_click=lambda _: file_picker.pick_files()),
                    file_path_text]),
            ft.Row([ft.IconButton(icon=ft.Icons.FOLDER, on_click=lambda _: folder_picker.get_directory_path()),
                    folder_path_text], expand=True),
            ft.Row(controls=[self.checkBox, self.kf_checkbox], expand=True),
        ...
        def on_excel_tab_change(_e, _output_folder_path_text, _tabs):
        ...
        excel_tab = ft.Tabs(
            ...
            tabs=[
                ft.Tab(text='无合并单元格单一表头拆分',
                       content=ft.Container(content=ft.Container(), padding=ft.padding.only(top=20))),
                ft.Tab(text='复杂表头拆分',
                       content=ft.Container(content=ft.Container(), padding=ft.padding.only(top=20)))
            ]
        )
        tab_page = ft.Tabs(
            ...
            tabs=[
                ft.Tab(text="基本拆分",
                       content=ft.Container(content=self._simple_split_excel(folder_path_text),
                                            padding=ft.padding.only(top=20))),
                ft.Tab(text="高级拆分",
                       content=ft.Container(content=excel_tab, padding=ft.padding.only(top=20)))
            ]
        )
        return ft.Column(controls=[excel_components, tab_page], spacing=25)
